...
variacoes_Neuronios = [2,6,14];
variacoes_Aprendizagem=[0.4, 0.1, 0.001];
variacoes_Func = ['tansig','logsig'];
variacoes_Algoritmo = ['trainoss','traingdm','trainlm'];
...
for p1 = 1:3;
    ...
    experimento = 0;
    for p2 = 1:3;
        numEscondidos = variacoes_Neuronios(p2) ;
        for p3 = 1:3;
            experimento = experimento+1;
            fprintf(fid,'Experimento: %6.5f \n',experimento);
            fprintf(fid,'versao: %6.5f \n',versao);
           ...
           % saidasTeste         = [novaSaidasTest; saidasTeste];
            ...
            rede.trainParam.lr       = variacoes_Aprendizagem(p3);

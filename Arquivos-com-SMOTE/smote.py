...
for y in range (28):
	...
	for x in range(c1_trainning_size): 
		...
		array = deepcopy(training_classe1);
		...
		for i in range(len(target.attributes)):
			...
			att_novo = atual + (proximo-atual)*alfa;
...
print(" Treinamento primeira parte ",len(training_classe1));
novos_valores =[];
...
print("Treinamento segunda parte ",len(training_classe1));
...
for y in range (28):
	...
	for x in range(c1_validation_size): 
		...
		array = deepcopy(validation_classe1);
		array[x].classification = -1;
		distances = euclidian(target,array);
		distances.sort(key = lambda x:x.distance,reverse = False) # da um sort no array de treinamento -> array.sort(key = lambda x:x.distance,reverse = False) 
		position = random.randint(0, 4);
...
print("Validacao primeira parte ",len(validation_classe1));
novos_valores =[];
...
print("Validacao segunda parte ",len(validation_classe1));
